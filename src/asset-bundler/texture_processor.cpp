...
texture_processor::texture_processor(options opts) : env_res(nullptr), opts(opts) {
    ...
    debug_report_callback = instance->createDebugReportCallbackEXT(
        cbco, nullptr, vk::DispatchLoaderDynamic(instance.get(), vkGetInstanceProcAddr)
    );
#endif
    ...
    for(auto pd : physical_devices) {
        ...
        for(uint32_t i = 0; i < qufams.size(); ++i) {
            ...
            if(qufams[i].queueCount > 0 && qufams[i].queueFlags & vk::QueueFlagBits::eGraphics
               && qufams[i].queueFlags & vk::QueueFlagBits::eCompute) {
    ...
    device = phy_device.createDeviceUnique(vk::DeviceCreateInfo{
        {}, 1, &queue_create_info, layer_count, layer_names, 0, nullptr, &device_features
    });
...
void texture_processor::submit_texture(texture_id id, texture_info* info) {
    info->img.mip_levels
        = (uint32_t)std::floor(std::log2(std::max(info->img.width, info->img.height))) + 1;
...
environment_info texture_processor::submit_environment(
 ...
 // TODO: these are fixed but should be based on some kind of quality level
    ...
    std::cout << "processing environment " << name << " " << width << "x" << height << " "
              << nchannels << "\n";
