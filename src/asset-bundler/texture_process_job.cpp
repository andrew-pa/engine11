...
texture_process_job::texture_process_job(
    vk::Device                            dev,
    ...
    : process_job(dev, cmd_pool),
      image_info{info->img.vulkan_create_info(
    ...
    memcpy(
        staging->cpu_mapped(),
        info->data,
        info->img.width * info->img.height * vk::blockSize(info->img.format)
    );

    cmd_buffer->begin(vk::CommandBufferBeginInfo{vk::CommandBufferUsageFlagBits::eOneTimeSubmit});

    // transition the first mip level to transfer destination to prepare to recieve staging buffer
    // data
...
void texture_process_job::generate_mipmaps() {
    ...
    vk::ImageBlit blit_info{
        vk::ImageSubresourceLayers{vk::ImageAspectFlagBits::eColor, 0, 0, 1},
        {vk::Offset3D{0, 0, 0},
                                   {(int32_t)image_info.extent.width, (int32_t)image_info.extent.height, 1}},
        vk::ImageSubresourceLayers{vk::ImageAspectFlagBits::eColor, 1, 0, 1},
        {vk::Offset3D{0, 0, 0},
                                   {glm::max((int32_t)image_info.extent.width / 2, 1),
          glm::max((int32_t)image_info.extent.height / 2, 1),
          1}},
    };
    ...
    // initial transition: transition the base mip level to source layout and the first new mip
    // level to destination layout
    cmd_buffer->pipelineBarrier(
        vk::PipelineStageFlagBits::eTransfer,
        vk::PipelineStageFlagBits::eTransfer,
        {},
    ...
    for(uint32_t i = 1; i < image_info.mipLevels; ++i) {
        ...
        if(i + 1 < image_info.mipLevels) {
            ...
            // transition the mip level we just wrote to from destination layout to source layout so
            // we can copy from it in the next iteration
            barrierDstToSrc.subresourceRange.setBaseMipLevel(blit_info.srcSubresource.mipLevel);
            // transition the next unwritten mip level from undefined to destination layout so we
            // can write to it in the next iteration
    ...
    cmd_buffer->copyImageToBuffer(
        img->get(), vk::ImageLayout::eTransferSrcOptimal, staging->get(), regions
    );
}
