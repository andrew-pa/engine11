...
struct inotify_watcher : public abstract_watcher {
    ...
    inotify_watcher(const std::filesystem::path& watched_file) {
        ...
        if(wd < 0) {
            ...
            throw std::runtime_error(std::string("failed to create inotify watch descriptor for ") + watched_file.string());
        }
...
std::filesystem::path copy_path(const std::filesystem::path& lib_path, uint64_t counter) {
	return (
        lib_path.parent_path() / std::filesystem::path(path_to_string(lib_path.stem()) + std::to_string(counter) + path_to_string(lib_path.extension())));
}
